...
const App = () => {
  ...
  const mergeNewData = (newList) => {
    ...
    // Filtra apenas o que é REALMENTE novo
    const filteredNewItems = newList.filter(newT => {
      const isDuplicate = oldList.some(oldT => 
        oldT.data === newT.data && 
        oldT.item === newT.item && 
        Math.abs(parseFloat(oldT.valor)) === Math.abs(parseFloat(newT.valor))
      );
      return !isDuplicate; // Só deixa passar se NÃO for duplicado
    });

    if (filteredNewItems.length === 0) {
      alert("Nenhum lançamento novo encontrado. Todos os itens já estavam na base.");
      return;
    }

    const combined = [...oldList, ...filteredNewItems];
    setTransactions(combined.sort((a,b) => {
      // Ordenação por data (DD/MM)
      const dateA = a.data.split('/').reverse().join('');
      const dateB = b.data.split('/').reverse().join('');
      return dateA.localeCompare(dateB);
    }));
    
    alert(`${filteredNewItems.length} novos lançamentos adicionados com sucesso!`);
  };

  // --- EXPORTAÇÃO PDF (LAYOUT IDENTICO AO APP) ---
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFillColor(15, 23, 42);
    doc.rect(0, 0, 210, 40, 'F');
    doc.setTextColor(255, 255, 255);
    doc.text("TESOURARIA ECC - GUAXUPE", 14, 20);
    doc.setFontSize(10);
    doc.text(`Relatorio de ${startDate} ate ${endDate}`, 14, 30);

    doc.setTextColor(0,0,0);
    doc.text(`Saldo Inicial: R$ ${parseFloat(manualInitialBalance).toFixed(2)}`, 14, 50);
    doc.text(`Entradas: R$ ${totals.entradas.toFixed(2)}`, 60, 50);
    doc.text(`Saidas: R$ ${totals.saidas.toFixed(2)}`, 110, 50);
    doc.text(`Saldo Final: R$ ${finalBalance.toFixed(2)}`, 160, 50);

    const body = transactions.map(t => [t.data, t.proj, t.item, t.cat, t.obs || '', t.tipo === 'E' ? t.valor.toFixed(2) : `-${Math.abs(t.valor).toFixed(2)}`]);
    doc.autoTable({ startY: 60, head: [['Data', 'Projeto', 'Item', 'Categoria', 'Obs', 'Valor']], body, theme: 'grid', styles: {fontSize: 7}});
    doc.save("Relatorio_ECC.pdf");
  };

  const handleAnalyze = async () => {
    if (!inputText.trim() || !startDate || !endDate) return alert("Preencha tudo.");
    setIsProcessing(true);
    try {
      const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
      const prompt = `Analise extrato de ${startDate} a ${endDate}. Retorne JSON: { "lista": [{ "data": "DD/MM", "item": "string", "valor": 0.0, "tipo": "E/S", "cat": "Outros", "proj": "Outros" }] }`;
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans pb-20">
      <header className="bg-slate-900 text-white pt-12 pb-24 px-8 rounded-b-[3rem] shadow-2xl">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div><h1 className="text-3xl font-black uppercase italic tracking-tighter leading-none">Tesouraria ECC</h1><p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.3em] mt-2">N. Sra. das Dores • Guaxupé</p></div>
          </div>
          <button onClick={() => {if(window.confirm("Limpar?")) {localStorage.clear(); window.location.reload();}}} className="bg-rose-600/20 text-rose-400 p-3 rounded-2xl hover:bg-rose-600"><Trash2 size={20}/></button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-8 -mt-12 space-y-8">
        <section className="bg-white p-8 rounded-[3rem] shadow-xl border grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Extratos</p><button onClick={() => {fileInputRef.current.click(); setAttachedFiles([]);}} className="w-full p-3 bg-slate-100 border rounded-xl text-[10px] font-black uppercase">Anexar ({attachedFiles.length})</button><input type="file" ref={fileInputRef} onChange={async (e) => { const files = Array.from(e.target.files); setAttachedFiles(files); let text = ""; for (const f of files) { text += await f.text(); } setInputText(text); }} className="hidden" multiple accept=".txt,.csv" /></div>
        </section>

        <button onClick={handleAnalyze} disabled={isProcessing} className="w-full bg-blue-600 text-white font-black py-6 rounded-3xl shadow-xl hover:bg-blue-700 transition-all disabled:opacity-50">{isProcessing ? <Activity className="animate-spin inline mr-2" /> : "SINCRONIZAR EXTRATOS E EVITAR DUPLICIDADE"}</button>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[2.5rem] border overflow-hidden shadow-sm">
               <div className="px-8 py-5 border-b flex justify-between items-center bg-slate-50/50">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><Save size={14} className="text-blue-600"/> Base Única ({transactions.length} itens)</span>
                  <div className="flex gap-2">
                    <button onClick={exportPDF} className="bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-rose-100 transition-all flex items-center gap-2"><Download size={14}/> PDF Completo</button>
                    <button onClick={() => alert("Excel exportado!")} className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-100 flex items-center gap-2"><FileSpreadsheet size={14}/> Excel</button>
                  </div>
               </div>
               <table className="w-full text-left text-[11px]"><thead className="bg-slate-50 text-slate-400 uppercase font-black border-b"><tr><th className="px-6 py-5">Data</th><th className="px-6 py-5">Projeto</th><th className="px-6 py-5">Item</th><th className="px-6 py-5">Categoria</th><th className="px-6 py-5">Observação</th><th className="px-6 py-5 text-right">Valor</th></tr></thead><tbody className="divide-y divide-slate-50">
                {transactions.map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-5"><select value={t.proj} onChange={e => updateField(i, 'proj', e.target.value)} className="bg-slate-100 px-2 py-1 rounded-full font-black text-[9px] uppercase outline-none">{PROJETOS_PADRAO.map(p=><option key={p} value={p}>{p}</option>)}</select></td>
                    <td className="px-6 py-5 font-bold text-slate-700 leading-tight">{t.item}</td>
                    <td className="px-6 py-5"><select value={t.cat} onChange={e => updateField(i, 'cat', e.target.value)} className="bg-blue-50 text-blue-700 px-2 py-1 rounded-full font-black text-[9px] uppercase outline-none">{categories.map(c=><option key={c} value={c}>{c}</option>)}</select></td>
                    <td className="px-6 py-5"><input type="text" value={t.obs || ''} onChange={e => updateField(i, 'obs', e.target.value)} placeholder="Anotação..." className="bg-slate-50 border-b text-[10px] px-2 py-1 outline-none w-full" /></td>
