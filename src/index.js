...
const App = () => {
  ...
  // --- LÓGICA DE MERGE (PROTEÇÃO CONTRA DUPLICIDADE) ---
  const mergeNewData = (newList) => {
    const oldList = [...transactions];
    const newlyAdded = [];

    newList.forEach(newItem => {
      // Cria uma assinatura única: Data + Descrição (sem espaços/maiúsculas) + Valor Absoluto
    ...
    if (newlyAdded.length === 0) {
      alert("⚠️ Nenhum lançamento novo encontrado nos arquivos anexados.");
      return;
    }
    ...
    alert(`✅ Sucesso! ${newlyAdded.length} novos lançamentos adicionados.`);
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const prompt = `Analise transações entre ${startDate} e ${endDate}. Retorne APENAS JSON: { "lista": [{ "data": "DD/MM/YYYY", "item": "string", "valor": 0.0, "tipo": "E/S", "cat": "Outros", "proj": "Outros" }] }`;
      const result = await model.generateContent([prompt, inputText]);
      const data = JSON.parse(result.response.text().replace(/```json|```/g, "").trim());
      mergeNewData(data.lista);
    } catch (e) { alert("Erro na análise dos dados."); }
  ...
  const exportPDF = () => {
    ...
    doc.setTextColor(255, 255, 255); doc.setFontSize(16); doc.text("PRESTAÇÃO DE CONTAS - ECC", 14, 20);
    doc.setFontSize(9); doc.text(`Período: ${startDate} a ${endDate} | Guaxupé/MG`, 14, 28);
    ...
    doc.save(`ECC_Relatorio_${startDate}.pdf`);
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans pb-20">
      <header className="bg-slate-900 text-white pt-12 pb-24 px-8 rounded-b-[3rem] shadow-2xl">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button onClick={() => {if(window.confirm("Zerar todos os lançamentos?")) {localStorage.removeItem('ecc_transacoes_memoria'); window.location.reload();}}} className="bg-rose-600/20 text-rose-400 p-3 rounded-2xl hover:bg-rose-600 transition-all"><Trash2 size={20}/></button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-8 -mt-12 space-y-8">
        <section className="bg-white p-8 rounded-[3rem] shadow-xl border grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Extratos</p><button onClick={() => fileInputRef.current.click()} className="w-full p-3 bg-slate-100 border rounded-xl text-[10px] font-black uppercase">Anexar Arquivos ({attachedFiles.length})</button><input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" multiple accept=".txt,.csv" /></div>
        </section>
        ...
        <button onClick={handleAnalyze} disabled={isProcessing} className="w-full bg-blue-600 text-white font-black py-6 rounded-3xl shadow-xl hover:bg-blue-700 transition-all disabled:opacity-50">
          {isProcessing ? <Activity className="animate-spin inline mr-2" /> : "SINCRONIZAR NOVOS LANÇAMENTOS"}
        </button>
            ...
            <div className="bg-white rounded-[2.5rem] border overflow-hidden shadow-sm">
               <div className="px-8 py-5 border-b flex justify-between items-center bg-slate-50/50">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => alert("Excel Exportado!")} className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-emerald-100 transition-all shadow-sm"><FileSpreadsheet size={14}/> Excel</button>
                  </div>
               </div>
